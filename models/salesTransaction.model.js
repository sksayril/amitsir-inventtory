...
salesTransactionSchema.index({ transactionNumber: 1 });
salesTransactionSchema.index({ transactionDate: 1 });
salesTransactionSchema.index({ debitParty: 1 });
salesTransactionSchema.index({ creditParty: 1 });
salesTransactionSchema.index({ status: 1 });
salesTransactionSchema.index({ createdBy: 1 });

// Static method to find by Transaction Number
salesTransactionSchema.statics.findByTransactionNumber = function(transactionNumber) {
  return this.findOne({ transactionNumber: transactionNumber });
};

// Static method to find by Date Range
salesTransactionSchema.statics.findByDateRange = function(startDate, endDate) {
  return this.find({
    transactionDate: {
      $gte: startDate,
      $lte: endDate
...
salesTransactionSchema.methods.toJSON = function() {
